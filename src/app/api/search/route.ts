...
export async function GET(request: Request) {
  try {
    ...
    const [clients, contacts, invoices, projects, tasks, news, tips, notes, contracts, proposals, users] = await Promise.all([
      ...
      prisma.siteArticle.findMany({ where: { deletedAt: null, title: contains }, take: 5, select: { id: true, title: true, status: true } }),
    ...
    const results: SearchResult[] = [
      ...
      ...news.map(n => ({ type: 'Haber', icon: '📰', title: n.title, link: `/site-yonetimi/haber/${n.id}` })),
