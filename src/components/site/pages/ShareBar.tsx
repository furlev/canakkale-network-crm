...
export default function ShareBar({ url, title }: { url: string; title: string }) {
  ...
  // Paylaşım hedefleri tek yerde — hem satır içi çubuk hem sabit ray kullanır.
  const iconX = (
  ...
  const shareLinks = [
    { key: 'x', label: 'X', aria: "X'te paylaş", href: `https://x.com/intent/tweet?text=${encodedTitle}&url=${encodedUrl}`, icon: iconX },
    { key: 'fb', label: 'Facebook', aria: "Facebook'ta paylaş", href: `https://www.facebook.com/sharer/sharer.php?u=${encodedUrl}`, icon: iconFb },
    { key: 'wa', label: 'WhatsApp', aria: "WhatsApp'ta paylaş", href: `https://wa.me/?text=${encodedTitle}%20${encodedUrl}`, icon: iconWa },
  ];

  return (
    <>
      {/* Satır içi çubuk (kanonik, akış içinde + mobil) */}
      <div className="p-share">
        <span className="p-share-label">Paylaş</span>
        {shareLinks.map(s => (
          <a key={s.key} className="p-share-btn" href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.aria}>
            {s.icon}
            {s.label}
          </a>
        ))}
      ...
      {/* Sabit dikey paylaşım rayı — yalnız geniş ekranda görünür (CSS), görsel
          zenginleştirme olduğu için ekran okuyuculardan gizli + tab dışı. */}
      <div className="s-share-rail" aria-hidden="true">
        <span className="s-share-rail-label">Paylaş</span>
        {shareLinks.map(s => (
          <a key={s.key} className="s-share-ico" href={s.href} target="_blank" rel="noopener noreferrer" tabIndex={-1} title={s.aria}>
            {s.icon}
          </a>
        ))}
        <button
          type="button"
          className={`s-share-ico ${copied ? 'is-copied' : ''}`}
          onClick={copy}
          tabIndex={-1}
          title="Bağlantıyı kopyala"
        >
          {copied ? (
            <svg viewBox="0 0 24 24" aria-hidden="true"><path d="M20 6 9 17l-5-5" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" /></svg>
          ) : (
            iconCopy
          )}
        </button>
